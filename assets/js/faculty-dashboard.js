...
function initialiseNavigation() {
  ...
  document.querySelectorAll("[data-open-panel]").forEach((button) => {
    button.addEventListener("click", () => {
      openDashboardPanel(button.dataset.openPanel);
    });
  });
...
(async () => {
  ...
  if (!["faculty", "ta"].includes(account.role)) {
    window.location.replace(
      account.role === "admin" ? "admin.html" : "index.html",
    );
    return;
  }
  ...
  if (
    !validation.ok ||
    !["faculty", "ta"].includes(validation.account.role)
  ) {
    localStorage.removeItem(ABHYASLAB_SESSION_KEY);
    window.location.replace("index.html");
    return;
  ...
  account.expiresAt =
    validation.expiresAt || account.expiresAt;

  localStorage.setItem(
    ABHYASLAB_SESSION_KEY,
  ...
  /*
   * Full course content is a Faculty permission. The Teaching Assistant
   * dashboard remains focused on assigned sections and student support.
   */
  if (!isFaculty) {
    document.getElementById("facultyCourseNav")?.remove();
    document.getElementById("facultyCourseQuick")?.remove();
  }
  ...
  document
    ...
    .addEventListener("click", () => secureLogout(account));
})();
